    ...
    <div class="pc-header-show">
      <div class="m-header">
        ...
        <div class="login-register">
          <el-button type="text" size="medium" @click="Dialog(true, 0)">登录</el-button>
          <el-button type="success" size="medium" round @click="showDialog(true, 1)">免费注册</el-button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    LoginOrRegister (visible, type) {
      ...
      this.type = type
      if (type === '0') {
        this.$emit('LoginOrRegister', this.visible, this.type)
      } else {
        this.$emit('doRegister', this.visible, this.type)
      }
    },
