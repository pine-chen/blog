...
const userInfo = async(id, username, newPassword, information) => {
    id = escape(id)
    username = escape(username)
    ...
    //sql语句有错误，应该是用户名和密码是varchar类型，要加引号。但我不会改。。。
    const sql = `update users set username=${username}, password=${newPassword}, information=${information} where id=${id}`
